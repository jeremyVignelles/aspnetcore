    ...
    internal static class TagHelperMatchingConventions
    {
        ...
        public static bool SatisfiesTagName(ReadOnlySpan<char> tagNameWithoutPrefix, TagMatchingRuleDescriptor rule)
        {
            ...
            if (rule.TagName != ElementCatchAllName &&
                rule.TagName != null &&
                !tagNameWithoutPrefix.Equals(rule.TagName.AsSpan(), rule.CaseSensitive ? StringComparison.Ordinal : StringComparison.OrdinalIgnoreCase))
            {
                return false;
            }
        ...
        public static bool SatisfiesParentTag(ReadOnlySpan<char> parentTagNameWithoutPrefix, TagMatchingRuleDescriptor rule)
        {
            if (rule.ParentTag != null &&
                !parentTagNameWithoutPrefix.Equals(rule.ParentTag.AsSpan(), rule.CaseSensitive ? StringComparison.Ordinal : StringComparison.OrdinalIgnoreCase))
            {
                return false;
            }
