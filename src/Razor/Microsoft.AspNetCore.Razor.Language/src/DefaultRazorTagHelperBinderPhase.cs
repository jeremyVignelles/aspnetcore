    ...
    internal class DefaultRazorTagHelperBinderPhase : RazorEnginePhaseBase, IRazorTagHelperBinderPhase
    {
        ...
        private static bool MatchesDirective(TagHelperDescriptor descriptor, string typePattern, string assemblyName)
        {
            ...
            if (typePattern.EndsWith("*", StringComparison.Ordinal))
            {
               ...
               return descriptor.Name.AsSpan().StartsWith(typePattern.AsSpan(0, typePattern.Length - 1), StringComparison.Ordinal);
            }
        ...
        internal class ComponentDirectiveVisitor : DirectiveVisitor
        {
            ...
            public ComponentDirectiveVisitor(string filePath, IReadOnlyList<TagHelperDescriptor> tagHelpers, string currentNamespace)
            {
                ...
                for (var i = 0; i < tagHelpers.Count; i++)
                {
                    ...
                    var typeName = tagHelper.GetTypeName().AsSpan();
            ...
            internal static bool IsTypeInNamespace(ReadOnlySpan<char> typeName, string @namespace)
            {
                ...
                return @namespace.Length == namespaceTextSpan.Length &&
                    typeName.Slice(namespaceTextSpan.Start, @namespace.Length).Equals(@namespace.AsSpan(), StringComparison.Ordinal);
            }
            ...
            internal static bool IsTypeInScope(ReadOnlySpan<char> typeNameSpan, string currentNamespace)
            {
                if (!TrySplitNamespaceAndType(typeNameSpan, out var namespaceTextSpan, out var _) || namespaceTextSpan.Length == 0)
                {
                    // Either the typeName is not the full type name or this type is at the top level.
                    return true;
                }

                var typeNamespace = GetTextSpanContent(namespaceTextSpan, typeNameSpan);
                if (typeNamespace.Length > currentNamespace.Length)
                {
                ...
                var typeNamespaceTokenizer = new StringTokenizer(typeNamespace, '.').GetEnumerator();
                var currentNamespaceTokenizer = new StringTokenizer(currentNamespace.AsSpan(), '.').GetEnumerator();

                while (typeNamespaceTokenizer.MoveNext())
                {
                    if (!currentNamespaceTokenizer.MoveNext())
                    {
                        return false;
                    }

                    if (!typeNamespaceTokenizer.Current.Equals(currentNamespaceTokenizer.Current, StringComparison.Ordinal))
                    {
                        return false;
                    }
                }
            ...
            internal static bool TrySplitNamespaceAndType(ReadOnlySpan<char> fullTypeName, out TextSpan @namespace, out TextSpan typeName)
            {
                @namespace = default;
                typeName = default;
            ...
            // Internal for testing.
            internal static ReadOnlySpan<char> GetTextSpanContent(TextSpan textSpan, ReadOnlySpan<char> s)
            {
                return s.Slice(textSpan.Start, textSpan.Length);
            }
        }
    }
}
